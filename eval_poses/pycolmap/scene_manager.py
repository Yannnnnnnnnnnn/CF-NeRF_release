    ...
    def _load_images_txt(self, input_file):
        ...
        with open(input_file, 'r') as f:
            ...
            for line in lines:
                ...
                data = line.split()
                # print('data',data, is_camera_description_line)
                if is_camera_description_line:
                    # print(np.array(list(map(float, data[1:5]))))
                    image_id = int(data[0])
                    image = Image(data[-1], int(data[-2]),
                                  Quaternion(np.array(list(map(float, data[1:5])))),
                                  np.array(list(map(float, data[5:8]))))
                    # print('Q',np.array(list(map(float, data[1:5]))))
                    # print('T',np.array(list(map(float, data[5:8]))))
