...
from .nerf_base import NeRF, SirenNeRF, GaussNeRF, DoubleNeRF
...
class SceneModel(nn.Module):
    def __init__(
        ...
        **dummy_kwargs):
        ...
        elif framework == 'GaussNeRF':
            base_cls = GaussNeRF
            input_ch = 3
            input_ch_views = 3 if use_viewdirs else 0
        else:
            raise RuntimeError("Please choose framework among: [NeRF, SirenNeRF, GaussNeRF]")
...
def create_model(
    ...
    model_kwargs = dict(
        ...
        use_fine_model=args.model.setdefault('use_fine_model', False), # does not use fine model
    ...
    if args.model.framework == 'NeRF' or args.model.framework == 'SirenNeRF' or args.model.framework == 'GaussNeRF':
